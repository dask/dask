...
Tuple: 
  A sequence of immutable Python objects. Tuples are sequences like lists. The difference between tuples and lists is that tuples use parenthesis and are immuatble (cannot be changed), whereas lists use square brackets and are mutable (can be changed).
