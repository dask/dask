...
* **Cutting-edge library developers:** Some developers use dask within their own libraries. Current projects include xray (climate science), scikit-image and scikit-allel (genomics). The developers of these libraries use dask to accelerate interactive analysis of large datasets in their domains.
...
* **Big pandas seekers:** Dask.dataframe is useful to process data files that are larger than can fit in available memory, providing some limited pandas-like ability. It covers part, but not all, of the pandas API.
