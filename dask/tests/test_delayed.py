...
from dask.delayed import delayed, to_task_dasks, compute, DelayedValue
...
def test_operators():
    a = delayed([1, 2, 3])
    assert a[0].compute(get=get_sync) == 1
...
def test_array_delayed():
    ...
    assert isinstance(val, DelayedValue)
...
def test_delayed_callable():
    f = delayed(inc, pure=True)
    assert f.pure is True
    v = f(1)
    assert v.dask == {v.key: (inc, 1)}

    assert f.dask == {f.key: inc}
    assert f.compute() == inc
