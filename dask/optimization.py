...
def cull(dsk, keys):
    ...
    convert = config.get("optimization.cull.convert-tasks")
...
def fuse_linear(dsk, keys=None, dependencies=None, rename_keys=True):
    ...
    **This function is mostly superseded by ``fuse``**
    ...
    convert = config.get("optimization.fuse_linear.convert-tasks")
...
def inline(dsk, keys=None, inline_constants=True, dependencies=None):
    ...
    convert = config.get("optimization.inline.convert-tasks")
...
def inline_functions(
    ...
    if keys:
        ...
        for k in keys:
            del dsk[k]
    elif config.get("optimization.inline.convert-tasks"):
...
def fuse(
    ...
    """
    convert = config.get("optimization.fuse.convert-tasks")
