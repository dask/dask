...
def empty_like(a, dtype=None, order="C", chunks=None, name=None, shape=None):
    ...
    if np.isnan(shape).any():
        return a.map_blocks(lambda x: np.empty_like(x, dtype=(dtype or x.dtype)))
...
def ones_like(a, dtype=None, order="C", chunks=None, name=None, shape=None):
    ...
    # generic map_blocks.
    if np.isnan(shape).any():
        return a.map_blocks(lambda x: np.ones_like(x, dtype=(dtype or x.dtype)))
...
def zeros_like(a, dtype=None, order="C", chunks=None, name=None, shape=None):
    ...
    # generic map_blocks.
    if np.isnan(shape).any():
        return a.map_blocks(lambda x: np.zeros_like(x, dtype=(dtype or x.dtype)))
