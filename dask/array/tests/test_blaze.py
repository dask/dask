...
dx = da.from_array(nx, blockshape=(4, 5))
sx = symbol('x', discover(nx))

ny = np.arange(600).reshape((30, 20))
dy = da.from_array(ny, blockshape=(5, 4))
sy = symbol('y', discover(ny))

na = np.arange(20)
dA = da.from_array(na, blockshape=(4,))
sa = symbol('a', discover(na))

nb = np.arange(30).reshape((30, 1))
db = da.from_array(nb, blockshape=(5, 1))
sb = symbol('b', discover(nb))
...
def test_compute():
    for expr in [2*sx + 1,
        ...
        result = compute(expr, dask_ns).compute()
...
def test_slicing_with_lists():
    ...
    dx = da.from_array(nx, blockshape=(2, 2))
    sx = symbol('x', discover(nx))
...
def test_slicing_on_boundary_lines():
    ...
    dx = da.from_array(nx, blockshape=(3, 3))
    sx = symbol('x', discover(nx))
    expr = sx[0, [1, 3, 9, 3]]
