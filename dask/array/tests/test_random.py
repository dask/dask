...
@pytest.mark.parametrize(
    "sd", [None, 42, np.random.PCG64DXSM, da.random.Generator(np.random.PCG64DXSM)], ids=type
)
...
def test_random_all_with_class_methods(generator_class, sz):
    generator_class().beta(1, 2, size=sz, chunks=3).compute()
    generator_class().binomial(10, 0.5, size=sz, chunks=3).compute()
    generator_class().chisquare(1, size=sz, chunks=3).compute()
    generator_class().exponential(1, size=sz, chunks=3).compute()
    generator_class().f(1, 2, size=sz, chunks=3).compute()
    generator_class().gamma(5, 1, size=sz, chunks=3).compute()
    generator_class().geometric(1, size=sz, chunks=3).compute()
    generator_class().gumbel(1, size=sz, chunks=3).compute()
    generator_class().hypergeometric(1, 2, 3, size=sz, chunks=3).compute()
    generator_class().laplace(size=sz, chunks=3).compute()
    generator_class().logistic(size=sz, chunks=3).compute()
    generator_class().lognormal(size=sz, chunks=3).compute()
    generator_class().logseries(0.5, size=sz, chunks=3).compute()
    generator_class().multinomial(20, [1 / 6.0] * 6, size=sz, chunks=3).compute()
    generator_class().negative_binomial(5, 0.5, size=sz, chunks=3).compute()
    generator_class().noncentral_chisquare(2, 2, size=sz, chunks=3).compute()
    generator_class().noncentral_f(2, 2, 3, size=sz, chunks=3).compute()
    generator_class().normal(2, 2, size=sz, chunks=3).compute()
    generator_class().pareto(1, size=sz, chunks=3).compute()
    generator_class().poisson(size=sz, chunks=3).compute()
    generator_class().power(1, size=sz, chunks=3).compute()
    generator_class().rayleigh(size=sz, chunks=3).compute()
    generator_class().triangular(1, 2, 3, size=sz, chunks=3).compute()
    generator_class().uniform(size=sz, chunks=3).compute()
    generator_class().vonmises(2, 3, size=sz, chunks=3).compute()
    generator_class().wald(1, 2, size=sz, chunks=3).compute()
    generator_class().weibull(2, size=sz, chunks=3).compute()
    generator_class().zipf(2, size=sz, chunks=3).compute()
    generator_class().standard_cauchy(size=sz, chunks=3).compute()
    generator_class().standard_exponential(size=sz, chunks=3).compute()
    generator_class().standard_gamma(2, size=sz, chunks=3).compute()
    generator_class().standard_normal(size=sz, chunks=3).compute()
    generator_class().standard_t(2, size=sz, chunks=3).compute()
