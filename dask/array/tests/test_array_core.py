...
def test_cumulative():
    ...
    assert_eq(x.nancumsum(axis=0), nancumsum(np.arange(20)))
    assert_eq(x.nancumprod(axis=0), nancumprod(np.arange(20)))

    a = np.random.random((20))
    rs = np.random.RandomState(0)
    a[rs.rand(*a.shape) < 0.5] = np.nan
    x = da.from_array(a, chunks=5)
    assert_eq(x.nancumsum(axis=0), nancumsum(a))
    assert_eq(x.nancumprod(axis=0), nancumprod(a))
    ...
    assert_eq(x.nancumsum(axis=0), nancumsum(a, axis=0))
    assert_eq(x.nancumsum(axis=1), nancumsum(a, axis=1))
    assert_eq(x.nancumprod(axis=0), nancumprod(a, axis=0))
    assert_eq(x.nancumprod(axis=1), nancumprod(a, axis=1))
    ...
    for axis in [0, 1, 2]:
        ...
        assert_eq(x.nancumsum(axis=axis), nancumsum(a, axis=axis))
        assert_eq(x.nancumprod(axis=axis), nancumprod(a, axis=axis))
