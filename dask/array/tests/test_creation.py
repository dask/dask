...
def test_eye():
    ...
    # default / "auto" single-block chunks with M != N (GH#11937)
    assert_eq(da.eye(2, M=3), np.eye(2, M=3))
    assert_eq(da.eye(3, M=2), np.eye(3, M=2))
    assert_eq(da.eye(2, chunks="auto", M=5), np.eye(2, M=5))
    assert_eq(da.eye(2, M=5, k=1), np.eye(2, M=5, k=1))
    assert_eq(da.eye(5, M=2, k=-1), np.eye(5, M=2, k=-1))
