    ...
    for dim, sizes in dimsizess.items():
        ...
        if not allow_rechunk:
            ...
            if (dim in core_shapes) and (chunksizes[0][0] < core_shapes[dim]):
                raise ValueError(
                    f"Core dimension `'{dim}'` consists of multiple chunks. To fix, rechunk into a single chunk along this "
                    "dimension or set `allow_rechunk=True`, but beware that this may increase memory usage significantly."
                )
...
class GUfuncLeafExpr(ArrayExpr):
    ...
    @functools.cached_property
    def _name(self):
        name = self.array._name.split("-")[-1]
        return f"{self.name_prefix}_{self.i}-{name}"
