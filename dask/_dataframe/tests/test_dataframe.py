...
import dask
import dask._dataframe.groupby
import dask.array as da
import dask.dataframe as dd
from dask import delayed
from dask._dataframe import _compat, methods
...
def test_describe_empty_tdigest():
    ...
    with warnings.catch_warnings():
        # dask._dataframe should probably filter this, to match pandas, but
...
def test_repartition_noop(type_ctor):
    ...
    # Top-level dask._dataframe method
    ...
    # Top-level dask._dataframe method applied to a Series
...
def test_info():
    ...
    assert buf.getvalue() == (
        "<class 'dask._dataframe.core.DataFrame'>\n"
...
def test_use_of_weakref_proxy():
    ...
    assert dask._dataframe.groupby._cov_chunk(df_pxy, "data")
    assert isinstance(
        dask._dataframe.groupby._groupby_apply_funcs(df_pxy, "data", funcs=[]),
        pd.DataFrame,
    )
