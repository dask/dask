...
from dask._dataframe._compat import PANDAS_GE_150, PANDAS_GE_200, PANDAS_GE_202
from dask._dataframe.io.parquet.core import get_engine
from dask._dataframe.io.parquet.utils import _parse_pandas_metadata
from dask._dataframe.optimize import optimize_dataframe_getitem
from dask._dataframe.utils import assert_eq, pyarrow_strings_enabled
...
def test_get_engine_pyarrow():
    from dask._dataframe.io.parquet.arrow import ArrowDatasetEngine

    assert get_engine("auto") is ArrowDatasetEngine
    assert get_engine("pyarrow") is ArrowDatasetEngine
    with pytest.warns(FutureWarning):
        assert get_engine("pyarrow-dataset") is ArrowDatasetEngine
    with pytest.warns(FutureWarning):
        assert get_engine("arrow") is ArrowDatasetEngine


@FASTPARQUET_MARK
def test_get_engine_fastparquet():
    from dask._dataframe.io.parquet.fastparquet import FastParquetEngine
...
def test_get_engine_fastparquet_only():
    ...
    with pytest.warns(
        ...
        assert get_engine("auto") is FastParquetEngine
...
def test_get_engine_third_party():
    ...
    from dask._dataframe.io.parquet.utils import Engine
...
def test_roundtrip_nullable_dtypes(tmp_path):
    ...
    fastparquet is unsupported: https://github.com/dask/fastparquet/issues/465
    """
    df = pd.DataFrame(
        {
...
def test_filters_v0(tmpdir, write_engine, read_engine):
    ...
    # Pyarrow supports full row-wise filtering; fastparquet does not.
...
def test_gpu_write_parquet_simple(tmpdir):
    ...
    from dask._dataframe.dispatch import pyarrow_schema_dispatch
