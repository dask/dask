...
def delegates(to=None, keep=False, but=None):
    ...
    included**.  The reason that required arguments (i.e. those without default
    parameters) are automatically excluded is that you should be explicitly
    implementing required arguments into your function's signature rather than
    relying on `delegates`.

    Additionally, you can exclude specific parameters from being included in
    the signature with the  `but` parameter.
    ...
    (https://github.com/fastai/fastcore/blob/ae8148c85a0c57cc7fd6aa29fa13bdbfbe59be22/fastcore/meta.py#L107-L126),
