...
class Bag(DaskMethodsMixin):
    ...
    def choice(self, size=1, replace=True):
        """Chooses size random elements from the bag with or without replacement.
...
def choice_map_partitions(x, size=1, replace=True):
    """
    Map function used in the Bag.choice method.
    Parameters
    ----------
    x : list
        List of element to sample.
    size : int, optional
        Number of elements to sample. Default is a single value is returned.
    replace : boolean, optional
        Whether the sample is with or without replacement
...
def choice_reduce(reduce_iter):
    """
    Reduce function used in the Bag.choice method.

    Parameters
    ----------
    reduce_iter : iterable
        Each element is a tuple coming generated by the choice_map_partitions function.
    ...
    for n_i, k_i in ns_ks:
        if k_i > 0:
            p_i = n_i / (k_i * n)
            p += [p_i] * k_i
    return np.random.choice(np_s, size=min(k, tot_ks), replace=replace, p=p)
