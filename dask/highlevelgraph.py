...
class Layer(Mapping):
    ...
    def cull(self, requested_keys: set, input_layers: set) -> tuple[Layer, Set]:
        """Cull unnecessary tasks from the Layer

        The underlying graph will only include the necessary
        tasks to produce the keys (indicies) included in `output_blocks`.
        Therefore, "culling" most layers should only require us to reset
        this parameter.
        ...
        ----------
        requested_keys: Set
            The known set of required keys. This set may contain
            keys that will be generated by other layers (and will
            therefore be ignored by this layer).
        input_layers: Set
            The names of layers that the current layer depends on.
        ...
        culled_layer: Layer
            A new (culled) layer.
        culled_deps: Set
            All keys that the new (culled) layer depends on.
        """
