...
def check_dask(dsk):
    if hasattr(dsk, 'dask'):
        ...
        if isinstance(dsk, dd.Series):
            assert isinstance(result, (pd.Index, pd.Series))
...
def test_arithmetic_frame():
    ...
    assert eq(d + 2, full + 2)
    assert eq(d * 2, full * 2)
    assert eq(d - 2, full - 2)
    assert eq(d / 2, full / 2)
    assert eq(d & True, full & True)
    assert eq(d | True, full | True)
    assert eq(d ^ True, full ^ True)
    assert eq(d // 2, full // 2)
    assert eq(d ** 2, full ** 2)
    ...
    assert eq(2 + d, 2 + full)
    assert eq(2 * d, 2 * full)
    assert eq(2 - d, 2 - full)
    assert eq(2 / d, 2 / full)
    assert eq(True & d, True & full)
    assert eq(True | d, True | full)
    assert eq(True ^ d, True ^ full)
    assert eq(2 // d, 2 // full)
    assert eq(2 ** d, 2 ** full)
