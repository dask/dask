...
@pytest.mark.parametrize("partition_sizes", [True, False])
def test_reset_index(partition_sizes):
    ...
    # If we know `partition_sizes` before `reset_index` then we know the partition-bounds of the auto-inc index
    # generated by `reset_index` (and therefore the resulting `divisions`)
    if partition_sizes:
        assert res.divisions == (0, 2, 3)
        assert res.partition_sizes == (2, 2)
    else:
        assert res.divisions == (None, None, None)
