...
def expr_table(expr: Expr, color: bool = True) -> ExprTable:
    ...
    table.add_column("Columns", style="dim", no_wrap=True)
    table.add_column("Dtypes", justify="right", style="dim", no_wrap=True)

    # Walk tree and add rows
    for node, prefix in walk_expr_with_prefix(expr, is_expr_child=_is_df_expr):
        ...
        try:
            npartitions = str(node.npartitions)
        except Exception:
            npartitions = "?"
