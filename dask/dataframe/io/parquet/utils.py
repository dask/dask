    ...
    def extract_filesystem(
        ...
        if filesystem is None:
            ...
            if "fs" in dataset_options:
                raise ValueError(
                    "Cannot specify a filesystem argument if the 'fs' dataset option is also defined."
                )
            fs = filesystem
...
def _split_user_options(**kwargs):
    ...
    if "file" in user_kwargs:
        ...
        warnings.warn(
            "Passing user options with the 'file' argument is now deprecated. Please use 'dataset' instead.",
            FutureWarning,
        )
