...
def test_read_json_basic(orient, tmp_path):
    f = os.path.join(tmp_path, "f.json")
...
def test_read_json_fkeyword(fkeyword, tmp_path):
...
@pytest.mark.parametrize("orient", ["split", "records", "index", "columns", "values"])
def test_write_json_basic(orient, tmp_path):

    fn = os.path.join(tmp_path, "1.json")
...
def test_to_json_with_get(tmp_path):
    ...
    ddf.to_json(tmp_path, compute_kwargs={"scheduler": my_get})
    assert flag[0]
    result = dd.read_json(os.path.join(tmp_path, "*"))
...
def test_read_chunked(block, tmp_path):
    fn = os.path.join(tmp_path, "1.json")
...
def test_json_compressed(compression, tmp_path):
    dd.to_json(ddf, tmp_path, compression=compression)
    actual = dd.read_json(os.path.join(tmp_path, "*"), compression=compression)
    assert_eq(df, actual.compute(), check_index=False)


def test_read_json_inferred_compression(tmp_path):
    fn = os.path.join(tmp_path, "*.json.gz")
