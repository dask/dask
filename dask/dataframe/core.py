...
Dask Name: {name}, {layers}"""
    ...
    def mean(
        ...
        if axis == 1:
            ...
            if axis is None and isinstance(self, DataFrame):
                result = s.sum() / n.sum()
            else:
    ...
    def var(
        ...
        if axis == 1:
            ...
            # pandas does not implement var on timedelta
    ...
    def kurtosis(
        ...
        """
        if axis is None:
            raise ValueError("`axis=None` isn't currently supported for `kurtosis`.")
        axis = self._validate_axis(axis)
    ...
    def quantile(self, q=0.5, axis=0, numeric_only=no_default, method="default"):
        ...
        if axis == 1:
            ...
            num = self._get_numeric_data() if numeric_only is True else self
            quantiles = tuple(
    ...
    def _describe_1d(
        ...
        elif is_datetime64_any_dtype(data._meta):
            return self._describe_numeric(
                data.dropna(),
                split_every=split_every,
                percentiles=percentiles,
...
def _sqrt_and_convert_to_timedelta(partition, axis, dtype=None, *args, **kwargs):
    ...
    for time_col, matching_val, unit in zip(time_cols, matching_vals, units):
        result = pd.to_timedelta(matching_val, unit=kwargs.get("unit", None))
        if kwargs.get("unit", None) is not None:
